...
class Tank_auth
{
	...
	// Inside the Tank_auth library
	public function get_user_email($user_id) {
		$user_data = $this->ci->users->get_user_by_id($user_id,1); // Assuming the users table contains the email field
		if ($user_data) {
			return $user_data->email;
		} else {
			return false; // User not found or email field does not exist
		}
	}

	public function get_user_first_name($user_id){
		$user_data = $this->ci->users->get_user_by_id($user_id,1);
		if ($user_data) {
	...
	public function get_user_last_name($user_id){
		$user_data = $this->ci->users->get_user_by_id($user_id,1);
		if ($user_data) {
